...
<div class="nmsf-wrapper" id="nmsf-page">
	<div class="nmsf-content">
		...
		<?php 
			...
			<input type="hidden" name="action" value="<?php echo esc_attr($class_ins->get_config('id')); ?>_settings_panel_action"/>
			<input type="hidden" name="nmsf_version" value="<?php echo esc_attr($class_ins->get_config('version')); ?>"/>
			...
			<div class="nmsf-header-area">
				<div class="row">
					...
					<div class="col-md-6 col-sm-6">
						<a href="<?php echo esc_url($migrate_url); ?>" class="button button-primary button-large nmsf-migrate-back-btn"><?php echo _e('Switch to Legacy Settings', $class_ins->get_config('plugin_domain')); ?></a>
			...
			<div class="row">
				<div class="nmsf-tabs-area nmsf-cols col-md-2">
					...
					<?php foreach ($tabs as $tab_id => $tab) {
						...
						if ($enable) {
						    ?>
							<div class="<?php echo implode(" ",$classes) ?>" data-tab-id="<?php echo esc_attr($tab_id); ?>">
				...
				<div class="nmsf-cols col-md-10">
					<div class="nmsf-panels-area">
						...
						?>
							...
							<div class="<?php echo implode(" ",$classes) ?> nmsf-panels-content" data-panel-id="<?php echo esc_attr($tab_id); ?>">
								...
								<div class="nmsf-panels-content-inner">
									...
									foreach ($panel_meta as $panel_id => $subtab_meta) {
										...
										$is_checked = '';
										if ($active == 'yes') { $is_checked = 'checked'; }
											
										if ($is_sabpanel) {
										?>
											<input type="radio" name="tab_<?php echo esc_attr($tab_id); ?>" class="nmsf-panel-handler" id="nmsf-<?php echo esc_attr($panel_id); ?>" <?php echo esc_attr($is_checked); ?>>
						      				<label for="nmsf-<?php echo esc_attr($panel_id); ?>" class="nmsf-label"><?php echo esc_html($title); ?></label>
															...
															<?php } else{ ?>
								      						...
								      						>
															    <th>
															        ...
															        <?php if ($tooltip) { ?>
															        	<span class="nmsf-tooltip" title="<?php echo $desc; ?>">
															        	...
															        	<span class="nmsf-ref-area">
															        		<?php if($ref_link != ''){ ?>
																        		<a href="<?php echo esc_url($ref_link); ?>" target="_blank">
															        		...
															        		<?php if($video_link != ''){ ?>
															        			<a class="nmsf-ref-video-popup" video-url="<?php echo esc_url($video_link); ?>">
															    ...
															    <td>
															    	...
															    	<?php if($hint != ''){ ?>
															    		<span class="nmsf-hint-area">
															    			<span><?php echo _e('Hint: ', $class_ins->get_config('plugin_domain')); ?></span>
				...
				<div class="row">
					<div class="col-md-6 col-sm-6">
						<h3>
							...
							<span class="badge badge-dark"><?php echo $class_ins->get_config('plugin_version'); ?></span> </h3>
					</div>
					<div class="col-md-6 col-sm-6">
						<input type="submit" class="button button-primary button-large" value="Submit"/>
					</div>
